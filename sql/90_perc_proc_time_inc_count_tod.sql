...
fire_count as (
	select 'fire' call_type, "Hour", percentile_cont(0.9) within group (order by process_time)  process_time, count(process_time) total
	from dumb_things_fire
	group by "Hour"
	order by call_type,"Hour"
),
med_count as (
	select 'medical' call_type, "Hour",percentile_cont(0.9) within group (order by process_time) , count(process_time) 
	from dumb_things_med
	group by "Hour"
	order by call_type,"Hour"
),
other_count as (
	select 'other' call_type, "Hour",percentile_cont(0.9) within group (order by process_time), count(process_time)
